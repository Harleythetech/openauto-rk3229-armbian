...
protected:
  void SetUp() override {
    mockConfiguration =
        std::make_shared<NiceMock<configuration::MockConfiguration>>();
...
TEST_F(ProjectionTest, TouchInputHandling) {
  ...
  ON_CALL(*mockConfiguration, hasTouchScreen()).WillByDefault(Return(true));
  ...
  // Call the touch event handler (we can only test that it doesn't crash with
  // our mocked objects)
