...
protected:
  ...
  void createTestConfigFile() {
    ...
    if (file.open(QIODevice::WriteOnly)) {
      ...
      // General settings
      configObject["HandednessOfTrafficType"] =
          static_cast<int>(HandednessOfTrafficType::LEFT_HAND_DRIVE);
...
TEST_F(ConfigurationTest, SaveAndLoadConfiguration) {
  ...
  configuration->setHandednessOfTrafficType(
  ...
  EXPECT_EQ(configuration->getHandednessOfTrafficType(),
            newConfiguration->getHandednessOfTrafficType());
  ...
  EXPECT_EQ(configuration->hideMenuToggle(),
            newConfiguration->hideMenuToggle());
